...
public class insercao {
    ...
    /* ---------- Ordenação por Inserção (chave: cidade) ---------- */

    static void inserirOrdenacao(Restaurante[] arr, int n) {
        for (int i = 1; i < n; i++) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        inserirOrdenacao(arr, n);
        long t1 = System.nanoTime();

        for (int i = 0; i < n; i++) System.out.println(arr[i].formatar());

        PrintWriter pw = new PrintWriter(new FileWriter(MATRICULA + "_insercao.txt"));
