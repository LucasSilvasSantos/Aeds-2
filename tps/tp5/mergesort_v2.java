...
public class mergesort_v2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = pesquisaCount - 1; i >= 0 && printed < 5; i--) {
            if (pesquisa[i] == null) continue;
            
            boolean alreadyUsed = false;
            for (int j = 0; j < usedCount; j++) {
                if (usedIds[j] == pesquisa[i].id) {
                    alreadyUsed = true;
                    break;
                }
            }
            ...
            if (!alreadyUsed) {
                ...
                usedIds[usedCount++] = pesquisa[i].id;
                printed++;
            }
        }
        MyIO.println("");
        ...
        for (int i = 0; i < pesquisaCount && printed < 5; i++) {
            if (pesquisa[i] == null) continue;
    ...
    // === MERGE SORT ===
    private static void mergeSort(Game[] a, int n) {
        if (n < 2) return;
        Game[] aux = new Game[n];
        mergesortRec(a, aux, 0, n - 1);
    }
    
    private static void mergesortRec(Game[] a, Game[] aux, int left, int right) {
    ...
    private static void merge(Game[] a, Game[] aux, int left, int mid, int right) {
        ...
        while (i <= mid && j <= right) {
            comparacoes++;
            if (compareByPrice(aux[i], aux[j]) <= 0) {
                a[k++] = aux[i++];
                movimentos++;
            } else {
                a[k++] = aux[j++];
    ...
    private static int compareByPrice(Game x, Game y) {
